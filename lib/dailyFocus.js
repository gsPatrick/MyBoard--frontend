...
function getDailyFocusStorageKey() {
  return buildTenantScopedKey(DAILY_FOCUS_STORAGE_KEY, getActiveTenantId());
}
...
export function shouldShowDailyFocus() {
  ...
  const lastShown = localStorage.getItem(getDailyFocusStorageKey());
  return lastShown !== toDateKey(new Date());
}

export function markDailyFocusShown() {
  if (typeof window === "undefined") return;
  localStorage.setItem(getDailyFocusStorageKey(), toDateKey(new Date()));
}
...
export function isOpenDemand(demand) {
  return demand?.status === "pending" || demand?.status === "in_progress";
}

export function sortFocusDemands(demands) {
