...
export const AI_PROVIDER_PRESETS = {
  ...
  custom: {
    ...
    docs_url: null,
    description:
      "CLIProxyAPI ou gateway OpenAI (/v1). Host:porta, Bearer token (api-keys) e model id de GET /v1/models.",
    accent: "#8b5cf6",
  },
};
...
export function mapAiSettingsToState(ai = {}) {
  ...
  for (const id of AI_PROVIDER_IDS) {
    ...
    forms[id] = {
      ...
      embedding_model: preset.customizable
        ? remote.embedding_model || preset.embedding_model || ""
...
export function buildSavePayload(activeProvider, form) {
  ...
  if (preset.customizable) {
    ...
    payload.embedding_model = form.embedding_model || null;
  } else {
