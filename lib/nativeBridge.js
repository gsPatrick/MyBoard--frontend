...
/* ---------- abrir ao iniciar o sistema (login items) ---------- */
export async function getLaunchAtLogin() {
  const n = api();
  if (!n?.getLaunchAtLogin) return false;
  try {
    return (await n.getLaunchAtLogin()) === true;
  } catch {
    return false;
  }
}
...
export async function setLaunchAtLogin(enabled) {
  const n = api();
  if (!n?.setLaunchAtLogin) return false;
  try {
    return (await n.setLaunchAtLogin(!!enabled)) === true;
  } catch {
    return false;
  }
}
